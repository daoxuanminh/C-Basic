...
int total_money_transaction_from (node *head)
{
    char from_account[MAX];
    scanf ("%s",from_account);
    node *p =head;
    int totalMoney=0;
    while (p != NULL)
    {
        if (strcmp(p->from_account,from_account)==0)
        {
            totalMoney += p->money;
        }
        p = p->pNext;
...
int checkQuery (char query[],node *head){
    ...
    {
        int kq = total_money_transaction_from(head);
        return kq;
    }
...
void printInf(node * head){
    ...
    while (p != NULL)
    {
        printf ("%s %s %d %s %s\n",p->from_account,p->to_account,p->money,p->time_point,p->atm);
        p = p->pNext;
    }
}
