...
int check(char *s)
{
    ...
    for (int i = 0; i < strlen(s); i++)
    {
        ...
        { //dong ngoac
            //if ()
            if (stackEmpty())
                return i; // c?n return v? trí b? l?i
            char x = pop();
            if (!match(x, s[i]))
                return i;
        }
...
void doc_tu_file(char file_name[], char s[])
{
...
{
    //nhap tu ban phim
    char str[30] = "6+7*(c/2*(6-3*c[2]))"; // A=6+7*(c/2*(6-3*c[2]))(
    /*
    fflush (stdin);
    gets (str);
    */
    char file_name[] = "bieuthuc.txt";
    doc_tu_file(file_name, str);
    int result = check(str);
    if (result != 0)
        if (result == -1)
        {
            printf("Thieu dau ngoac dong!\n");
        }
        if (result > 0 && check_loi_ngoac())
        else
